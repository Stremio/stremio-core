        ...
        let fx = match msg {
            ...
            Msg::Action(Action::UserOp(action)) => match action.to_owned() {
                ...
                ActionUser::PullAndUpdateAddons => match &self.content.auth {
                    ...
                    None => {
                        // Local update based on the OFFICIAL_ADDONS
                        self.content.addons =
                            addons_upgrade_local(&OFFICIAL_ADDONS, &self.content.addons);
                        Effects::none()
                    }
                },
                ...
                ActionUser::LibSync
                | ActionUser::LibUpdate(_)
                | ActionUser::AddToLibrary { .. }
                | ActionUser::RemoveFromLibrary { .. } => Effects::none().unchanged(),
            },
