...
pub fn update_streaming_server_urls<E: Env + 'static>(
    server_urls: &mut ServerUrlsBucket,
    status: &CtxStatus,
    msg: &Msg,
) -> Effects {
    match msg {
        ...
        Msg::Internal(Internal::CtxAuthResult(auth_request, result)) => match (status, result) {
                ...
                if loading_auth_request == auth_request =>
            {
                ...
                *server_urls = next_server_urls;
                Effects::msg(Msg::Internal(Internal::StreamingServerUrlsBucketChanged))
            }
            _ => Effects::none().unchanged(),
        },
        Msg::Internal(Internal::StreamingServerUrlsBucketChanged) => {
            Effects::one(push_server_urls_to_storage::<E>(server_urls)).unchanged()
        }
        _ => Effects::none().unchanged(),
    }
}

fn push_server_urls_to_storage<E: Env + 'static>(server_urls: &ServerUrlsBucket) -> Effect {
    let uid: Option<String> = server_urls.uid.clone();

    EffectFuture::Sequential(
        E::set_storage(STREAMING_SERVER_URLS_STORAGE_KEY, Some(server_urls))
