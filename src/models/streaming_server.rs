...
use crate::types::streaming_server::{
    ArchiveCreateResponse, ArchiveStreamOptions, ArchiveStreamRequest, GetHTTPSResponse,
    NetworkInfo, Settings, SettingsResponse, Statistics,
};
...
pub async fn update_stream_source_streaming_url<E: Env + 'static>(
    ...
    match (transport_url, stream_source) {
        (
            ...
            Ok(StreamSource::Url { url: stream_url })
        }
        (
            Some(streaming_server_url),
            StreamSource::Zip {
        ...
        (_, stream_source) => Ok(stream_source),
    }
}
