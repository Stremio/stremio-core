    ...
    pub fn new<E: Env + 'static>(profile: &Profile) -> (Self, Effects) {
        ...
        (
            Self {
                ...
                base_url: None,
                server_urls_bucket: ServerUrlsBucket::new(profile.uid().to_owned()),
                remote_url: None,
        ...
        match msg {
            Msg::Action(Action::StreamingServer(ActionStreamingServer::Reload)) => {
                ...
                let server_urls_bucket_effects = eq_update(
                    ...
                    ServerUrlsBucket::new(ctx.profile.uid().to_owned()),
                );
                ...
                if self.selected.transport_url != ctx.profile.settings.streaming_server_url =>
            {
                ...
                self.base_url = None;
                self.server_urls_bucket = ServerUrlsBucket::new(ctx.profile.uid().to_owned());
