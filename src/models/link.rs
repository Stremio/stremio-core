...
use crate::types::api::{
    fetch_api, APIError, APIResult, APIVersion, LinkCodeResponse, LinkDataResponse, LinkRequest,
};
...
fn create_code<E: Env + 'static>() -> Effect {
    EffectFuture::Concurrent(
        fetch_api::<E, _, _, LinkCodeResponse>(APIVersion::V2, &LinkRequest::Create)
...
fn read_data<E: Env + 'static>(code: &str) -> Effect {
    EffectFuture::Concurrent(
        fetch_api::<E, _, _, LinkDataResponse>(
            APIVersion::V2,
            &LinkRequest::Read {
                code: code.to_owned(),
            },
