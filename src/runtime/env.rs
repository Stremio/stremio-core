        ...
        Self::get_storage::<u32>(SCHEMA_VERSION_STORAGE_KEY)
            .and_then(|schema_version| async move {
                ...
                if schema_version == 24 {
                    migrate_storage_schema_to_v25::<Self>()
                        .map_err(|error| EnvError::StorageSchemaVersionUpgrade(Box::new(error)))
                        .await?;
                    schema_version = 25;
                }
...
fn migrate_storage_schema_to_v25<E: Env>() -> TryEnvFuture<()> {
    E::get_storage::<serde_json::Value>(PROFILE_STORAGE_KEY)
        .and_then(|mut profile| {
            match profile
                .as_mut()
        ...
        .and_then(|_| E::set_storage(SCHEMA_VERSION_STORAGE_KEY, Some(&25)))
        .boxed_env()
}
...
mod test {
    ...
    #[tokio::test]
    async fn test_migration_from_22_to_23() {
        {
            let _test_env_guard = TestEnv::reset().expect("Should lock TestEnv");
            let profile_before = json!({
                "settings": {}
            ...
            // setup storage for migration
            set_profile_and_schema_version(&profile_before, 22);

            // migrate storage
            migrate_storage_schema_to_v23::<TestEnv>()
                .await
                .expect("Should migrate");
    ...
    #[tokio::test]
    async fn test_migration_from_24_to_25() {
        {
            let _test_env_guard = TestEnv::reset().expect("Should lock TestEnv");
            let profile_before = json!({
                "settings": {}
            ...
            // setup storage for migration
            set_profile_and_schema_version(&profile_before, 24);

            // migrate storage
            migrate_storage_schema_to_v25::<TestEnv>()
                .await
                .expect("Should migrate");
