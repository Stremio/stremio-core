    ...
    E::get_storage::<serde_json::Value>(PROFILE_STORAGE_KEY)
        .and_then(|mut profile| {
                ...
                .and_then(|settings| settings.as_object_mut())
            {
                Some(settings) => {
                    settings.insert(
                        "exoPlayerCache".to_owned(),
                        serde_json::Value::Number(1073741824.into()),
                    );
                    E::set_storage(PROFILE_STORAGE_KEY, Some(&profile))
                }
                _ => E::set_storage::<()>(PROFILE_STORAGE_KEY, None),
...
mod test {
    ...
    use crate::runtime::env::migrate_storage_schema_to_v15;
    ...
    async fn test_migration_from_14_to_15() {
        ...
        let migrated_profile = json!({
            "settings": {
                "exoPlayerCache": 1073741824
            }
        });

        set_profile_and_schema_version(&init_profile, 14);
